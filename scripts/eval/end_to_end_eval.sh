...
python prototypes/prepare_environment_for_end_to_end_eval.py --dir "$DIR"
# Run the model on the validation set
python src/bin/fb_predict.py -i "$DIR/reference/val" -w "$WEIGHTS" -o "$DIR/output" -p **.jpg -f --gpu cuda:0 --no-crops --no-overviews --verbose
# Compare the predictions with the ground truth
python src/bin/fb_eval.py -p "$DIR/output/**/**.json" -g "$DIR/reference/gt/instances_default.json" -I "$DIR/reference/val" -P -o "$DIR/eval"
...
python prototypes/clean_up_end_to_end_eval.py --dir "$DIR" --clear-all
