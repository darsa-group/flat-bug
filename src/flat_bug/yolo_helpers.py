...
def offset_box(
    boxes: torch.Tensor, offset: torch.Tensor, max_x: int | float | None = None, max_y: int | float | None = None
):
    """Move bounding boxes."""
    m = 4 / offset.shape[0]
...
def offset_mask(
    mask: torch.Tensor, offset: torch.Tensor, new_shape: tuple[int, int] | list[int] | None = None, max_size=700
):
    """Offset (move) binary masks in a new coordinate system."""
    # Due to memory use, it is beneficial to restrict the maximum size of the masks.
    # A 700x700 boolean tensor uses ~0.5 MB of memory
    n, h, w = mask.shape
    ...
    # If the mask is larger than the maximum size,
    # clamp it by downscaling it such that the largest dimension is max_size
    if clamp_factor > 1:
        # F.interpolate(
...
def merge_tile_results(
    ...
    boxes = torch.cat(
        [
            ...
            for r, o in zip(results, box_offsetters)
    ...
    if exclude_masks:
        ...
        masks = torch.cat(
            [
                ...
                for r, o in zip(results, mask_offsetters)
...
def stack_masks(masks: list[Masks | torch.Tensor], orig_shape: tuple[int, int] | list[int] | None = None):
    """Stacks a list of ultralytics `Masks` objects (or `torch.Tensor`) into a single `Masks` object.
    ...
    Args:
        ...
        antialias: Flag indicating whether to use antialiasing when resizing the masks. Defaults to False.
    ...
    assert all([isinstance(m, torch.Tensor) for m in tensor_masks]), (
        f"'masks' must be a list of torch.Tensor, not {type(tensor_masks[0])}"
    )
...
def crop_masks(masks: torch.Tensor, boxes: torch.Tensor):
...
def process_mask(
    ...
    Args:
        ...
        upsample: Flag indicating whether to upsample the mask to the original image size. Default is False.
...
def expand_bottom_right(mask: torch.Tensor):
    ...
    Returns:
        ...
        where an extra pixel is added above next to left/top edges of the region of 1s.
    """
...
def clip_boxes(boxes: V, shape: tuple[int, int]) -> V:
...
def postprocess(
    ...
    Args:
        ...
        nms: The type of non-maximum suppression to use.
            Defaults to 0. 0 is no NMS, 1 is standard NMS, 2 is fancy NMS and 3 is mask NMS.
        valid_size_range: The range of valid sizes for the bounding boxes in pixels.
            Defaults to None (no valid size range).
        edge_margin: The minimum gap between the edge of the image and the bounding box in pixels
            for a prediction to be considered valid. Defaults to None (no edge margin).
    ...
    if _is_end2end_output(preds):
        ...
        if max_det != 0:
            p = p.gather(
                2, torch.argsort(p[:, 4, :], dim=1, descending=True)[:, :max_det].unsqueeze(1).expand(-1, p.size(1), -1)
            )
    ...
    for i, (pred, _) in enumerate(zip(p, range(len(imgs)))):
        ...
        if edge_margin is not None and edge_margin > 0:
            close_to_edge = (
                (boxes[:, :2] < edge_margin).any(dim=1) | 
                (boxes[:, 2:] > (tile_size - edge_margin)).any(dim=1)
            )
