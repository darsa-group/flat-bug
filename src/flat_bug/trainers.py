...
def _custom_end_to_end_validation(self: "FlatBugSegmentationTrainer"):
    ...
    val_pattern = "({})".format("|".join([os.path.basename(f).replace(".", r"\.") for f in val_paths]))
...
def replaceattr(
    o: object, name: str, value, filters: list = [lambda _: True], exclude_prefix="_", label: str = "object"
):
...
def apply_overrides_to_checkpoint(overrides):  # noqa: D103
    ...
    for k, v in overrides.items():
        if not k.startswith("fb_") and v is not None:
            replaceattr(
                resume_ckpt, k, v, [lambda x: isinstance(x, (str, int, float)) or x is None], label="resume_ckpt"
            )
    # Change save dir
    if "name" not in overrides:
        overrides["name"] = (
            list(findattr(resume_ckpt, "name", [lambda x : isinstance(x, str)]).values()) or ["train"]
        )[0]
    if "project" not in overrides:
        overrides["project"] = (
            list(findattr(resume_ckpt, "project", [lambda x : isinstance(x, str)]).values()) or ["runs/segment"]
        )[0]
    ...
    replaceattr(
        ...
        "save_dir",
        new_save_dir,
        [lambda x: isinstance(x, (str, int, float)) or x is None],
        label="resume_ckpt",
    )
...
class FlatBugSegmentationTrainer(SegmentationTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._synth_kwargs = {
            ...
            # Deliberately NOT fb_max_instances: that is often set to 9999 to keep
            # every instance of a dense real image, whereas here it bounds how much
        ...
        # The custom arguments must be removed before calling super.__init___
        updated_overrides = remove_custom_fb_args(overrides or {})
    ...
    def get_dataloader(
        self, dataset_path: str, batch_size: int | None = 16, rank: int = 0, mode: str = "train"
    ) -> InfiniteDataLoader:
